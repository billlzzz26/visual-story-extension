...
`;
      ...
      expect(result.issues.find(i => i.code === 'MISSING_CLIMAX')?.severity).toBe('error');
      expect(result.issues.find(i => i.code === 'MISSING_CLIMAX')?.suggestion).toContain('Act 3');
    });
    ...
    it('should validate act distribution (25-50-25 rule)', () => {
      ...
      const act1Pct = (result.analysis.actBalance.act1 / result.analysis.eventCount) * 100;
      const act2Pct = (result.analysis.actBalance.act2 / result.analysis.eventCount) * 100;
      const act3Pct = (result.analysis.actBalance.act3 / result.analysis.eventCount) * 100;
    ...
    it('should detect Act 1 imbalance', () => {
      ...
      expect(result.issues.find(i => i.code === 'ACT1_IMBALANCE')?.message).toContain('25%');
    });
    ...
    it('should detect Act 2 imbalance', () => {
      ...
      graph.events.forEach((e, i) => {
        if (i < 5) e.act = 1;
        else if (i < 7) e.act = 2; // Only 2 events in Act 2
        else e.act = 3;
      });
    ...
    it('should validate in strict mode and check character motivations', () => {
      ...
      expect(result.issues.filter(i => i.code === 'NO_MOTIVATION').length).toBe(2); // Both characters
    });
    ...
    it('should check character transformation arc in strict mode', () => {
      ...
      expect(result.issues.find(i => i.code === 'NO_ARC')?.severity).toBe('warning');
    });
...
`;
    ...
    it('should recommend expanding Act 2 if too small', () => {
      ...
      graph.events.forEach((e, i) => {
        if (i < 6) e.act = 1;
        else if (i < 8) e.act = 2; // Only 2 out of 13 events
        else e.act = 3;
      });
...
`;
    ...
    it('should handle edge case with no events', () => {
      const emptyGraph: StoryGraph = {
        meta: { title: 'Empty', createdAt: '', updatedAt: '', version: '1.0.0' },
        characters: [],
        conflicts: [],
        events: [],
        relationships: [],
  ...
  describe('Canvas Exporter - Enhanced', () => {
    ...
    it('should create event nodes with proper structure', () => {
      ...
      const eventNodes = canvas.nodes.filter((n: any) => n.type === 'event');
    ...
    it('should create character nodes with proper structure', () => {
      ...
      const charNodes = canvas.nodes.filter((n: any) => n.type === 'character');
    ...
    it('should create conflict nodes with proper structure', () => {
      ...
      const conflictNodes = canvas.nodes.filter((n: any) => n.type === 'conflict');
    ...
    it('should create sequential edges between events', () => {
      ...
      const seqEdges = canvas.edges.filter((e: any) => e.id.startsWith('seq_'));
    ...
    it('should apply correct styles for different importance levels', () => {
      ...
      const climaxEvents = canvas.nodes.filter((n: any) =>
        n.type === 'event' && n.data.importance === 'climax'
      );

      if (climaxEvents.length > 0) {
        expect(climaxEvents[0].style.backgroundColor).toBe('#ffebee');
        expect(climaxEvents[0].style.borderColor).toBe('#c62828');
        expect(climaxEvents[0].style.borderWidth).toBe(4);
      }
    });
    ...
    it('should animate high-strength relationships', () => {
      ...
      if (relEdges.length > 0) {
        const highStrength = relEdges.find((e: any) => e.animated === true);
    ...
    it('should position events by act', () => {
      ...
      const act1Events = canvas.nodes.filter((n: any) => n.type === 'event' && n.data.act === 1);
      const act2Events = canvas.nodes.filter((n: any) => n.type === 'event' && n.data.act === 2);

      if (act1Events.length > 0 && act2Events.length > 0) {
        // Act 2 events should be further right (x position)
        expect(act2Events[0].position.x).toBeGreaterThan(act1Events[0].position.x);
      }
    });
  });
...
`;
    ...
    it('should properly escape HTML entities', () => {
      ...
      expect(html).toContain('<title>Test &lt;script&gt;alert("xss")&lt;/script&gt; - bl1nk Story Dashboard</title>');
      expect(html).toContain('<h1 class="text-4xl font-bold mb-2">Test &lt;script&gt;alert("xss")&lt;/script&gt;</h1>');
    });
  ...
  describe('Edge Cases and Regression Tests', () => {
    ...
    it('should handle conflict intensity edge values', () => {
      ...
      if (graph.conflicts.length > 0) {
        ...
        const conflictNode1 = canvas1.nodes.find((n: any) => n.type === 'conflict');
        ...
        const conflictNode2 = canvas2.nodes.find((n: any) => n.type === 'conflict');
    ...
    it('should handle events without importance classification', () => {
      ...
      const eventNode = canvas.nodes.find((n: any) => n.id === graph.events[0].id);
    ...
    it('should handle relationship strength at boundaries', () => {
      ...
      if (graph.relationships.length > 0) {
        ...
        const canvas1 = toCanvasJSON(graph);
        const edge1 = canvas1.edges.find((e: any) => e.id === 'rel_0');
        expect(edge1.animated).toBe(false);

        graph.relationships[0].strength = 8;
        const canvas2 = toCanvasJSON(graph);
        const edge2 = canvas2.edges.find((e: any) => e.id === 'rel_0');
