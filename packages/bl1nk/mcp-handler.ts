...
import { executeStoryTool, BL1NK_VISUAL_TOOLS, Schemas } from './index.js';
 ...
 */
export function initializeMcpServer(server: McpServer, config: McpHandlerConfig): void {
  ...
  if (debug) {
    console.log(`[EXA-MCP] Registering ${toolsToRegister.length} tools: ${toolsToRegister.map(t => t.name).join(', ')}`);
  }
