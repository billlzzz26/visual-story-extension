...
import { mockStoryGraph, mockValidationResult, mockMermaidDiagram } from './lib/mock-data';
    ...
    <div className="flex h-screen w-screen bg-slate-50 text-slate-800 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-6">
          <ViewContent activeView={activeView} graph={graph} validation={validation} />
        </div>
      </main>
    </div>
  );
...
function ViewContent({ activeView, graph, validation }: ViewContentProps): React.ReactElement {
  switch (activeView) {
    case 'editor':
      ...
      return (
        <div className="space-y-6">
          <div className="bg-gradient-to-r from-indigo-600 to-blue-500 p-6 rounded-xl text-white shadow-lg">
            ...
            <p className="text-indigo-100">{graph.meta.genre || 'General'} • v{graph.meta.version}</p>
          </div>
...
function EditorView({ graph }: { graph: typeof mockStoryGraph }): React.ReactElement {
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        <h2 className="text-lg font-semibold text-slate-800 mb-4">Characters</h2>
...
function InsightsView({ graph, validation }: { graph: typeof mockStoryGraph; validation: typeof mockValidationResult }): React.ReactElement {
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-indigo-600 to-blue-500 p-6 rounded-xl text-white shadow-lg">
        <h2 className="text-2xl font-bold mb-2">{graph.meta.title}</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <h2 className="text-lg font-semibold text-slate-800 mb-4">Character Roster</h2>
          <div className="divide-y divide-slate-100">
            {graph.characters.map((character) => (
              <div key={character.id} className="py-3 flex justify-between items-center">
                <div>
                  <p className="font-bold text-slate-900">{character.name}</p>
                  <p className="text-xs text-slate-500">{character.traits.slice(0, 3).join(', ')}</p>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <h2 className="text-lg font-semibold text-slate-800 mb-4">Core Conflicts</h2>
          <div className="space-y-4">
            {graph.conflicts.map((conflict) => (
              <div key={conflict.id} className="p-4 bg-slate-50 rounded-xl">
                <div className="flex justify-between mb-2">
                  <span className="text-xs font-bold text-indigo-600 uppercase">{conflict.type}</span>
                  <span className="text-xs text-slate-400">Act {conflict.actIntroduced}</span>
                </div>
                <p className="text-sm text-slate-800 font-medium">{conflict.description}</p>
              </div>
            ))}
          </div>
        </div>
