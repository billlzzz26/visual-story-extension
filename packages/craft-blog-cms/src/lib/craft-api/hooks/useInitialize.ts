...
import type { CollectionInitConfig, CraftFolder, DisplayItem, InitConfig } from '../types'
import { readSnapshot } from './useLocalItems'

function isAuthError(err: unknown): boolean {
  if (err && typeof err === 'object' && 'name' in err && (err as { name: string }).name === 'CraftApiError') {
...
function findFolderByName(folders: CraftFolder[], name: string): CraftFolder | undefined {
...
function collectBlockIds(block: { id: string; children?: Array<{ id: string; children?: unknown[] }> }): Set<string> {
...
async function resolveSeedItems(
  ...
  if (options?.localPersistenceKey) {
    try {
      ...
      if (snapshot?.items?.length) {
        ...
        if (collectionKey) items = items.filter((item) => item.fields._collectionKey === collectionKey)
        if (items.length > 0) {
          return items.map((item) => {
            const props = { ...item.fields }
            delete props._collectionKey
  ...
  const mockSlug = collectionKey ? (options?.slug || '') + '-' + collectionKey : options?.slug
  ...
  return mockItems.map((item) => ({ title: item.title, properties: item.properties }))
}
...
async function seedCollectionIfEmpty(
  ...
  const itemsRes = await client.getCollectionItems(collectionId).catch(catchNonAuth(null))
  if (itemsRes && itemsRes.items.length === 0) {
    const seedItems = await resolveSeedItems(collectionConfig, options, collectionKey)
    await client.createCollectionItems(collectionId, seedItems).catch(catchNonAuth(null))
  }
}
...
export function useInitialize(
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      try {
        const foldersRes = await client.getFolders().catch(catchNonAuth({ items: [] as CraftFolder[] }))
        ...
        const docsRes = await client
          .getDocuments(resolved.folderId ? { folderId: resolved.folderId } : {})
          .catch(catchNonAuth({ items: [] as Array<{ id: string; title: string; isDeleted: boolean }> }))

        const existing = docsRes.items.find((d) => !d.isDeleted && d.title?.toLowerCase() === init.document.toLowerCase())
        if (existing) {
          resolved.documentId = existing.id
        } else {
          const created = await client.createDocument(init.document, resolved.folderId).catch((err): null => {
            if (isAuthError(err)) throw err
            return null
          })
