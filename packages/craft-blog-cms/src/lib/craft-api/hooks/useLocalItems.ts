...
export function useLocalItems(items: DisplayItem[], actions?: DataSourceActions) {
  ...
  const updateField = useCallback(
    (itemId: string, fieldKey: string, value: unknown) => {
      ...
      const nextItems = currentItems.map((item) =>
        item.id === itemId ? { ...item, fields: { ...item.fields, [fieldKey]: value } } : item
      )
      ...
      actions
        ...
        .catch((err) => {
          console.error('[useLocalItems] updateField FAILED — rolling back:', err)
          setLocalItems(prev)
        })
    },
  ...
  const updateTitle = useCallback(
    (itemId: string, title: string) => {
      if (!actions?.update) return
      const prev = currentItems
      const nextItems = currentItems.map((item) => (item.id === itemId ? { ...item, title } : item))
      const updatedItem = nextItems.find((item) => item.id === itemId)
      setLocalItems(nextItems)
      if (!updatedItem) return
      actions
        ...
        .catch((err) => {
          console.error('[useLocalItems] updateTitle FAILED — rolling back:', err)
          setLocalItems(prev)
        })
    },
    [currentItems, actions]
  ...
  return { currentItems, setLocalItems, updateField, updateTitle, deleteItem, createItem }
}
