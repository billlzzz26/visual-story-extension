...
export type TextStyle = 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6' | 'p' | 'caption' | 'page'
...
export type DataSourceType = 'tasks' | 'collections' | 'documents' | 'blocks' | 'search' | 'upload' | 'multi-collection'
...
export interface DisplayItem {
  ...
  sourceType: 'task' | 'document' | 'collection-item' | 'block' | 'search-result' | 'upload-item' | 'folder'
  raw?: unknown
  craftLink?: string
}
...
export interface DataSourceActions {
  update?: (items: Array<{ id: string; [key: string]: unknown }>) => Promise<void>
  create?: (item: Record<string, unknown>) => Promise<DisplayItem | void>
  delete?: (ids: string[]) => Promise<void>
  upload?: (file: File) => Promise<unknown>
  updateEnumValues?: (fieldKey: string, values: string[]) => Promise<FieldSchema[]>
}
