...
import type { AppConfig, CollectionItem, CraftBlock, CraftDocument, CraftFolder, CraftTask } from './types'

const GENERIC_TASKS = ["Review incoming items","Update workspace data","Prepare next milestone","Follow up with collaborators","Check recent changes","Plan next action"] as const
const GENERIC_DOCUMENTS = ["Article Workspace","Notes","Reference","Archive"] as const
const GENERIC_SEARCH = ["Recent workspace note","Important record","Pinned reference","Example content"] as const
...
const DENSE_LAYOUTS = new Set(['swimlane', 'matrix', 'heatmap', 'calendar', 'timeline', 'gantt', 'schedule'])
...
export function generateMockItems(
  ...
  const titleSamples =
    mockTitles?.length
      ? mockTitles
      : Array.from({ length: count }, (_, i) => (collectionName || 'Item') + ' ' + (i + 1))
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    for (const [key, prop] of Object.entries(schema.properties)) {
      if (prop.enum?.length) {
        ...
        if (/amount|value|price|budget|cost/i.test(key)) properties[key] = Math.floor(rand() * 5000) + 100
        else if (/rating/i.test(key)) properties[key] = (i % 5) + 1
        else if (/progress|score|count|total|qty|quantity/i.test(key)) properties[key] = Math.floor(rand() * 100) + 1
        ...
        if (mockImages?.length) properties[key] = mockImages[i % mockImages.length]
        else properties[key] = 'https://picsum.photos/seed/' + nameKey + '-' + key + '-' + i + '/400/300'
...
function createMockBlocks(documentName: string): CraftBlock[] {
  return [
    { id: 'b1', type: 'text', textStyle: 'h2', markdown: documentName || 'Overview' },
    {
      id: 'b2',
      type: 'text',
      textStyle: 'p',
      markdown: 'This demo bundle ships with lightweight mock data so you can explore the app immediately.'
    },
    { id: 'b3', type: 'text', textStyle: 'h3', markdown: 'Getting started' },
    {
      ...
      textStyle: 'p',
      listStyle: 'bullet',
      markdown: 'Connect your workspace to replace mock data with live Craft data.'
    }
  ]
}
...
export function createMockClient(appConfig?: AppConfig): CraftApiClient {
  ...
  const mockCollectionItems = collectionSchema
    ? generateMockItems(collectionSchema, collectionName, slug, undefined, layout, mockTitles, mockImages)
    : []
  ...
  if (collectionName) allCollectionEntries.push({ id: 'mock-collection', name: collectionName })

  const folders: CraftFolder[] = [
    { id: 'folder-main', name: appConfig?.init?.folder || 'Workspace', documentCount: GENERIC_DOCUMENTS.length }
  ]
  ...
  const mock = {
    ...
    getBlocks: async () => ({ id: 'root', type: 'text' as const, children: blocks }),
    insertBlocks: async (newBlocks: Array<Partial<CraftBlock>>) => ({
      items: newBlocks.map((block, index) => ({ id: 'mock-block-' + (index + 1), type: 'text' as const, ...block }))
    }),
    updateBlocks: async () => ({ items: [] }),
    deleteBlocks: async () => ({ items: [] }),
    uploadFile: async (file: File) => ({ id: 'upload-' + file.name, type: 'file' as const, markdown: file.name }),
    ...
    getCollectionSchema: async (id: string) => {
      if (multiCollectionMap[id]) return { type: 'object' as const, properties: multiCollectionMap[id].schema }
      return collectionSchema
        ? { type: 'object' as const, properties: collectionSchema.properties }
        : { type: 'object' as const, properties: {} }
    },
    getCollectionItems: async (id: string) => {
      if (multiCollectionMap[id]) return { items: multiCollectionMap[id].items }
      return { items: mockCollectionItems }
    },
    createCollectionItems: async (_id: string, items: Array<{ title: string; properties?: Record<string, unknown> }>) => ({
      items: items.map((item, index) => ({ id: 'new-' + (index + 1), title: item.title, properties: item.properties || {} })) as CollectionItem[]
    }),
    ...
    createCollection: async (name: string) => ({
      ...
      schema: {
        ...
        properties: {} as Record<string, { type: string; title?: string; enum?: string[] }>
      }
    }),
    updateCollectionSchema: async () => ({ type: 'object' as const, properties: collectionSchema?.properties || {} }),
    search: async (query: string) => {
      ...
      const recipeItems = [
        ...
        {
          blockId: 'search-2',
          markdown: 'Thai Green Curry — coconut milk, basil, green beans, chicken',
          pageBlockPath: [
      ...
      const defaultItems = GENERIC_SEARCH.map((markdown, index) => ({
        ...
        markdown,
        pageBlockPath: [{ id: 'doc-' + (index + 1), content: GENERIC_DOCUMENTS[index % GENERIC_DOCUMENTS.length] }]
      }))

      return {
        items:
          /recipe|ingredient|meal|food|cook|dish|pantry/.test(q) || /recipe/.test(slug || '')
            ? recipeItems
            : defaultItems
      }
    },
