...
import { StorageLocationProvider, type StorageLocation } from '../runtime/StorageLocationContext'
...
export function GenericTemplate({ client, config, runtimeControls }: GenericTemplateProps) {
  const localPersistenceKey = config.slug ? `${config.slug}:${config.layout}` : undefined
  ...
  const isCraftStorageActive = (runtimeControls?.storageLocation ?? 'browser') === 'craft' && isConnected
  ...
  const enrichedResources = resources
    ? {
        ...
        blockUrlTemplate: isCraftStorageActive ? connectionInfo?.urlTemplates?.blockUrl : undefined
      }
    : undefined
  const { items, schema, actions, collectionSchemas, collectionActions, loading, error } = useDataSource(
    client,
    config.dataSource,
    enrichedResources,
    config.init
  ...
  const localPersistenceEnabled =
    (runtimeControls?.showStorageControls ?? false) && (runtimeControls?.storageLocation ?? 'browser') === 'browser'
  ...
  const storageContextValue = useMemo(
    () => ({
      ...
      onSelectStorageLocation: runtimeControls?.onSelectStorageLocation ?? (() => {}),
  ...
  const initFailed =
    ((config.dataSource.type === 'collections' || config.dataSource.type === 'upload') &&
      config.init?.collection &&
      !resources.collectionId) ||
    (config.dataSource.type === 'blocks' && config.init?.document && !resources.documentId) ||
    (config.dataSource.type === 'multi-collection' &&
      config.init?.collections?.length &&
      (!resources.collectionIds || Object.keys(resources.collectionIds).length === 0))

  if (error || initFailed) {
    return (
      <div className="rounded-lg bg-amber-50 px-4 py-3 text-sm text-amber-700">
        Could not load data from your workspace. Make sure you have granted full space access when connecting.
      </div>
    )
  }
  ...
  if (!items.length && !hasOwnEmptyState) return <EmptyState message="No items found in your workspace yet." />
  ...
  const enrichedOptions = collectionSchemas
    ? { ...config.config, _collectionSchemas: collectionSchemas, _collectionActions: collectionActions }
    : config.config
