...
export class CraftApiClient {
  ...
  constructor(accessToken: string, onError?: CraftApiErrorHandler, onRefresh?: CraftApiRefreshHandler) {
  ...
  private async request<T>(path: string, options: RequestInit = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      if ((status === 401 || (body.error as string) === 'invalid_token') && this.onRefresh) {
        ...
        if (newToken) {
          ...
          const retryBody = (await retryResponse.json().catch(() => ({}))) as Record<string, unknown>
      ...
      const error = new CraftApiError(status, (body.error as string) || response.statusText, path, body)
      this.onError?.(error)
      throw error
    }
  ...
  async getBlocks(
    params: { id?: string; date?: string; maxDepth?: number; fetchMetadata?: boolean } = {}
  ): Promise<CraftBlock> {
  ...
  async insertBlocks(
    ...
    position: { position: 'start' | 'end' | 'before' | 'after'; pageId?: string; siblingId?: string }
  ...
  async deleteBlocks(blockIds: string[]): Promise<ListResponse<{ id: string }>> {
  ...
  async getDocuments(
    params: { location?: string; folderId?: string; fetchMetadata?: boolean } = {}
    ...
    if (params.fetchMetadata) searchParams.set('fetchMetadata', 'true')
    const query = searchParams.toString()
    return this.request<ListResponse<CraftDocument>>(`/documents${query ? `?${query}` : ''}`)
  }

  async createDocument(title: string, parentFolderId?: string): Promise<CraftDocument> {
  ...
  async deleteDocuments(documentIds: string[]): Promise<ListResponse<{ id: string }>> {
  ...
  async getCollections(): Promise<ListResponse<{ id: string; name: string }>> {
    return this.request<ListResponse<{ id: string; name: string }>>('/collections')
  }
  ...
  async getCollectionSchema(id: string): Promise<CollectionSchema> {
    ...
    const fieldProps = raw?.properties?.items?.items?.properties?.properties?.properties
  ...
  async getCollectionItems(id: string): Promise<ListResponse<CollectionItem>> {
    return this.request<ListResponse<CollectionItem>>(`/collections/${id}/items`)
  }
  ...
  async createCollectionItems(
    ...
    return this.request<ListResponse<CollectionItem>>(`/collections/${collectionId}/items`, {
      method: 'POST',
  ...
  async updateCollectionItems(
    collectionId: string,
    items: Array<{ id: string; title?: string; properties?: Record<string, unknown> }>
  ): Promise<ListResponse<CollectionItem>> {
    return this.request<ListResponse<CollectionItem>>(`/collections/${collectionId}/items`, {
      method: 'PUT',
  ...
  async createTasks(
    tasks: Array<{
      markdown: string
      taskInfo?: { state?: 'todo' | 'done' | 'canceled'; scheduleDate?: string; deadlineDate?: string }
      location: { type: 'document' | 'inbox' | 'dailyNote'; documentId?: string; date?: string }
    }>
  ...
  async createCollection(
    name: string,
    schema: { properties: Record<string, { type: string; title: string; enum?: string[] }> },
    position: { position: 'start' | 'end'; pageId?: string } = { position: 'end' }
  ): Promise<{ collectionBlockId: string; name: string; schema: CollectionSchema }> {
    ...
    const apiProperties = Object.entries(schema.properties).map(([, prop]) => {
      const apiProp: Record<string, unknown> = {
        type: prop.enum ? 'singleSelect' : prop.type === 'string' ? 'text' : prop.type,
        name: prop.title
      }
      if (prop.enum) {
  ...
  async deleteCollectionItems(collectionId: string, idsToDelete: string[]): Promise<ListResponse<{ id: string }>> {
    return this.request<ListResponse<{ id: string }>>(`/collections/${collectionId}/items`, {
      method: 'DELETE',
      body: JSON.stringify({ idsToDelete })
    })
  }

  async updateCollectionSchema(collectionId: string, schema: CollectionSchema): Promise<CollectionSchema> {
    return this.request<CollectionSchema>(`/collections/${collectionId}/schema`, {
      method: 'PUT',
  ...
  async getTasks(scope: TaskScope, documentId?: string): Promise<ListResponse<CraftTask>> {
  ...
  async updateTasks(
    tasks: Array<{
      id: string
      markdown?: string
      taskInfo?: { state?: 'todo' | 'done' | 'canceled'; scheduleDate?: string | null; deadlineDate?: string | null }
    }>
  ): Promise<ListResponse<CraftTask>> {
    return this.request<ListResponse<CraftTask>>('/tasks', {
      method: 'PUT',
  ...
  async deleteTasks(idsToDelete: string[]): Promise<ListResponse<{ id: string }>> {
  ...
  async uploadFile(
    ...
    if (!response.ok) {
      ...
      if ((response.status === 401 || (body.error as string) === 'invalid_token') && this.onRefresh) {
        const newToken = await this.onRefresh()
        if (newToken) {
          this.accessToken = newToken
          const retryResponse = await fetch(url, {
      ...
      const error = new CraftApiError(response.status, (body.error as string) || response.statusText, '/upload', body)
      this.onError?.(error)
      throw error
    }
  ...
  async search(pattern: string): Promise<ListResponse<SearchResult>> {
    return this.request<ListResponse<SearchResult>>(`/search?pattern=${encodeURIComponent(pattern)}`)
  }

  async searchDocuments(pattern: string): Promise<ListResponse<SearchResult>> {
    return this.request<ListResponse<SearchResult>>(`/search/documents?pattern=${encodeURIComponent(pattern)}`)
  }
}
...
export class CraftApiError extends Error {
  ...
  constructor(status: number, message: string, path: string, body?: Record<string, unknown>) {
    super(message)
  ...
  get humanMessage(): string {
    if (this.status === 400) return 'Invalid request — check your input and try again.'
    if (this.status === 401) return 'Session expired — please reconnect.'
    if (this.status === 403) return "You don't have permission for this action."
    if (this.status === 404) return 'The requested item was not found.'
    if (this.status === 409) return 'Conflict — the item was modified elsewhere.'
    if (this.status === 422) return 'Validation failed — some fields have invalid values.'
    if (this.status === 429) return 'Too many requests — please wait a moment.'
    if (this.status >= 500) return "Something went wrong on Craft's end. Try again shortly."
