...
export function CraftApp({ appConfig }: CraftAppProps) {
  ...
  const storageLocation: StorageLocation = preferred === 'craft' && (isConnected || isConnecting) ? 'craft' : 'browser'
  ...
  const activeClient = storageLocation === 'craft' && client ? client : mockClient

  return (
    <div className="w-full">
      {error && <div className="mb-3 rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-700">{error}</div>}
      <GenericTemplate
        client={activeClient}
        config={appConfig}
        runtimeControls={{
