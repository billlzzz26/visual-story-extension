import { CraftApiError, type CraftApiClient } from '../client'
import type { CraftBlock, DataSourceConfig, DataSourceResult, FieldSchema } from '../types'
...
async function fetchRootBlockWithRetry(
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      lastError = err
      const isRetryable =
        err instanceof TypeError || (err instanceof CraftApiError && (err.status === 404 || err.status === 429))
      if (!isRetryable || attempt === 3) break
      console.warn(`[fetchBlocks] Root block lookup failed on attempt ${attempt}/3; retrying…`, err)
      await sleep(250 * attempt)
    }
  }

  if (lastError instanceof CraftApiError && lastError.status === 404) {
    console.warn('[fetchBlocks] Root block not found yet; returning an empty block list for now.', lastError)
    return null
  }

  if (lastError instanceof CraftApiError && lastError.status === 429) {
    console.warn('[fetchBlocks] Block lookup hit rate limit; returning an empty block list for now.', lastError)
    return null
  }

  if (lastError instanceof TypeError) {
    console.warn('[fetchBlocks] Block lookup failed to fetch; returning an empty block list for now.', lastError)
    return null
  }
...
export async function fetchBlocks(
  ...
  const rootBlock = await fetchRootBlockWithRetry(client, { id: documentId, maxDepth: config.maxDepth ?? 3 })
    ...
    actions: {
      update: async (updates) => {
        await client.updateBlocks(
          updates.map((u) => ({
            id: u.id,
            markdown: (u.markdown as string | undefined) ?? (u.title as string | undefined),
            color: u.color as string | undefined
          }))
        )
      },
      create: async (item) => {
        const result = await client.insertBlocks([{ type: 'text', markdown: (item.title as string) || '' }], {
          position: 'end',
          pageId: documentId
        })
