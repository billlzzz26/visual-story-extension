 ...
 */
export async function readSnapshot(key: string): Promise<PersistedSnapshot | null> {
  ...
  return await new Promise((resolve, reject) => {
    ...
    request.onerror = () => reject(request.error || new Error('Failed to read persisted data'))
...
async function migrateLegacyCookieToIndexedDb(key: string): Promise<void> {
  ...
  const candidateKeys = [slug ? `api_example_demo_${slug}` : '', `api_example_demo_${key.replace(':', '_')}`].filter(
    Boolean
  )
