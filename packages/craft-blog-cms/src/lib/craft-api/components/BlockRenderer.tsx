    ...
    if (isTodo) {
      return (
        <label className="flex items-start gap-2 py-0.5">
          <input type="checkbox" checked={isDone} readOnly className="mt-1 h-4 w-4 rounded border-gray-300" />
          <Tag
  ...
  return (
    <div style={indent} className="group/block relative">
      ...
      {block.children?.map((child) => (
        <BlockNode key={child.id} block={child} depth={depth + 1} onUpdate={onUpdate} onDelete={onDelete} />
      ))}
    </div>
  )
}
...
export function BlockRenderer({
  ...
  return (
    <div className="space-y-0.5">
      {blocks.map((block) => (
        <BlockNode key={block.id} block={block} onUpdate={onUpdate} onDelete={onDelete} />
      ))}
    </div>
  )
}
