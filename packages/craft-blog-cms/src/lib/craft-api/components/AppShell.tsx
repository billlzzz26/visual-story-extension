...
import { useContainerBreakpoint, type Breakpoint } from '../hooks/useContainerBreakpoint'
import { useStorageLocation, type StorageLocation } from '../runtime/StorageLocationContext'
...
export function AppShell({ children }: AppShellProps) {
  ...
  if (isNested) {
    return (
      <div ref={containerRef} className="@container h-full w-full overflow-auto">
        {children(breakpoint)}
      </div>
    )
  }
    ...
    >
      <div className="flex h-full flex-col">
          ...
          <div className="border-b border-gray-100 bg-gray-50/90 px-3 py-2">
            ...
            {localPersistenceEnabled && (
              <div className="mt-2 flex flex-wrap items-start justify-between gap-2 rounded-md border border-amber-200 bg-amber-50 px-2.5 py-1.5 text-[11px] text-amber-800">
                <span>
                  Data saved on this device may still be deleted by the browser or device (for example after cleanup,
                  storage pressure, or browser resets/reloads). Connect Craft to store data more safely in your space.
                </span>
                <button
        ...
        <div className="min-h-0 min-w-0 flex-1 overflow-auto">
          <AppShellContext.Provider value={true}>{children(breakpoint)}</AppShellContext.Provider>
        </div>
      </div>
