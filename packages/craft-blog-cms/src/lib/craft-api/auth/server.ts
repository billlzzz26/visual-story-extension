...
function isAllowedCallbackPath(pathname: string): boolean {
  ...
  return pathname.startsWith('/') && !pathname.startsWith('/api/') && !pathname.includes('..')
}
...
export function validateOAuthRedirectUri(redirectUri: string): string {
  ...
  if (!getAllowedOrigins().has(parsed.origin) && !isLocalOrigin(parsed.origin)) {
  ...
  if (parsed.hash) {
    throw new OAuthValidationError('redirectUri must not include a hash fragment')
  }
