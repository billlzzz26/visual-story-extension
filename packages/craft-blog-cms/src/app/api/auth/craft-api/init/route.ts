...
import { OAuthValidationError, applyNoStoreHeaders, validateOAuthRedirectUri } from '@/lib/craft-api/auth/server'
...
export async function POST(request: NextRequest) {
  try {
    ...
    cookieStore.set(OAUTH_COOKIE, JSON.stringify({ codeVerifier, clientId, redirectUri, state }), {
      httpOnly: true,
    ...
    const response = NextResponse.json({ authorizeUrl: authorizeUrl.toString(), state })
    applyNoStoreHeaders(response.headers)
    return response
  } catch (error) {
    console.error('[craft-api/init]', error)
    const response = NextResponse.json({ error: error instanceof Error ? error.message : 'Failed' }, { status: error instanceof OAuthValidationError ? 400 : 500 })
    applyNoStoreHeaders(response.headers)
    return response
  }
}
