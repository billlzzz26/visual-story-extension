...
export async function GET(request: NextRequest) {
  const code = (request.nextUrl.searchParams.get('code') || '').replace(/[^a-zA-Z0-9_-]/g, '')
  const state = (request.nextUrl.searchParams.get('state') || '').replace(/[^a-zA-Z0-9_-]/g, '')
...
if (window.opener) { window.opener.postMessage(msg, origin); window.close(); }
  ...
  const response = new NextResponse(html, {
    headers: {
      ...
      'Content-Security-Policy': "default-src 'none'; script-src 'unsafe-inline'; base-uri 'none'; form-action 'none'; frame-ancestors 'self'",
