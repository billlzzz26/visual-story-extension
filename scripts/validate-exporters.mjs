...
assert(graph.characters.length === 2, `expected 2 characters, got ${graph.characters.length}`);
assert(graph.events.length === 11, `expected 11 events, got ${graph.events.length}`);
assert(graph.conflicts.length === 2, `expected 2 conflicts, got ${graph.conflicts.length}`);
...
assert(canvas.nodes.length === graph.events.length + graph.characters.length + graph.conflicts.length,
...
assert(seqEdges.length === graph.events.length - 1, 'correct sequential edge count');
...
const xssGraph = buildInitialGraph('Title: Test <script>alert("xss")</script>\nCharacter: Hero, role: protagonist');
