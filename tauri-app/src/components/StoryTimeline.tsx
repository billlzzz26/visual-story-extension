...
export function StoryTimeline({ events }: StoryTimelineProps): React.ReactElement {
  const sortedEvents = [...events].sort((a, b) => {
    if (a.act !== b.act) return a.act - b.act;
    return a.sequenceInAct - b.sequenceInAct;
  });

  const currentAct = { 1: 'Act I - Setup', 2: 'Act II - Confrontation', 3: 'Act III - Resolution' };

  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
      <h2 className="text-lg font-semibold text-slate-800 mb-4">Story Timeline</h2>
            ...
            <div key={event.id} className="flex items-start gap-4">
              ...
              <div className="flex-1 pb-4">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-medium text-slate-500">{currentAct[event.act as keyof typeof currentAct]}</span>
